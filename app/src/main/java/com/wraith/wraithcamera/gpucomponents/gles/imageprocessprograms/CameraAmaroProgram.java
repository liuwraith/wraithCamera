 ...
 */
public class CameraAmaroProgram extends Texture2dProgram{
    private int mMixLocation = -1;
    private float mMix = 0.7f;
    public static final String AMARO_FRAGMENT_SHADER =
            ...
            + "    highp vec4 texel2 = texture2D(inputImageTexture, vTextureCoord);\n"
            ...
            + "    gl_FragColor = vec4(mix(texel2.rgb, mapped.rgb, mixturePercent),1.0);\n"
            + "}";
    ...
    public CameraAmaroProgram(Context context) {
        mDefaultTextureTarget = GLES11Ext.GL_TEXTURE_EXTERNAL_OES;
        mFilterType = FilterType.FILTER_AMARO;
        mProgramHandle = GlUtil.createProgram(VERTEX_SHADER, AMARO_FRAGMENT_SHADER);

        if(mProgramHandle == 0) {
            throw new RuntimeException("Unable to create program");
        }
        mInputImageTexture2Handler = createTextureObject(context, R.mipmap.sdk_blackboard_1024);
        mInputImageTexture3Handler = createTextureObject(context, R.mipmap.overlay_map);
        mInputImageTexture4Handler = createTextureObject(context, R.mipmap.sdk_amaro_map);
        initParams();
    }
