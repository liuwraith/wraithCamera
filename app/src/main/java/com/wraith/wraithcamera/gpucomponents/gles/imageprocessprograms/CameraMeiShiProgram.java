 ...
 */
public class CameraMeiShiProgram extends Texture2dProgram{
    private static final String FragmentShader =
            ...
            "   result = mix(temp, clamp(adjustCoffieStyle(result), EPSILON, oneSubEpsilon), coffieRatio);\n"+
            "   gl_FragColor = vec4(result, 1.0);\n" +
            "} \n" +
            "";

    //shadows: Increase to lighten shadows, from 0.0 to 1.0, with 0.0 as the default.
    private float mShadow = 0.3f;
    //highlights: Decrease to darken highlights, from 0.0 to 1.0, with 1.0 as the default.
    private float mHighLight = 0.15f;
    // from -1 to 1 default 0
    private float mContrast = 0.25f;
    //from -1 to 1 default 0
    private float mSaturation = 0.08f;
    //from 0 to 1 default 0
    private float mBrightness = 0.15f;

    private float mCoffiesRatio = 0.5f;
    ...
    private int mCoffieRatioLoc = -1;

    private int mMeiShiTextureHandler = -1;
    private int mMeiShiTextureLoc = -1;

    public CameraMeiShiProgram(Context context) {
        mDefaultTextureTarget = GLES11Ext.GL_TEXTURE_EXTERNAL_OES;
        mFilterType = FilterType.FILTER_MEISHI;
        mProgramHandle = GlUtil.createProgram(VERTEX_SHADER, FragmentShader);

        if(mProgramHandle == 0) {
            throw new RuntimeException("Unable to create program");
        }
        mMeiShiTextureHandler = createTextureObject(context, R.mipmap.cccc);
        initParams();
    }
    ...
    public void release(){
        super.release();
        if(mMeiShiTextureHandler != -1) {
            GLES20.glDeleteTextures(1, new int[]{mMeiShiTextureHandler}, 0);
            mMeiShiTextureHandler = -1;
        }
    }
    ...
    protected void initParams(){
        super.initParams();
        mMeiShiTextureLoc = GLES20.glGetUniformLocation(mProgramHandle, "inputImageTexture2");
    ...
    public void draw(float[] mvpMatrix, FloatBuffer vertexBuffer, int firstVertex,
        ...
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, mMeiShiTextureHandler);
        GLES20.glUniform1i(mMeiShiTextureLoc, 1);
