 ...
 */
public class CameraMildProgram extends Texture2dProgram {
    private static final String FragmentShader =
            ...
            "uniform mediump float coffieRatio;\n"+
    ...
    //from -1 to 1 default 0
    private float mSaturation = -0.26f;
    ...
    public CameraMildProgram(Context context) {
        mDefaultTextureTarget = GLES11Ext.GL_TEXTURE_EXTERNAL_OES;
        mFilterType = FilterType.FILTER_MILD;
        mProgramHandle = GlUtil.createProgram(VERTEX_SHADER, FragmentShader);

        if(mProgramHandle == 0) {
            throw new RuntimeException("Unable to create program");
        }
        mInputImageTexture2Handler = createTextureObject(context, R.mipmap.rookiecam_mild);
        initParams();
    }
