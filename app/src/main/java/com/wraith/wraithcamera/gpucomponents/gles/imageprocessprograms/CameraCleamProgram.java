 ...
 */
public class CameraCleamProgram extends Texture2dProgram {
    private static final String FragmentShader =
            ...
            "    vec3 contrastColor = vec3(0.2, 0.315, 0.2) + (originalColor.rgb  - vec3(0.2, 0.3, 0.2)) * (1.0 + contrast);\n" +
            "    return vec3(originalColor.r, contrastColor.g, contrastColor.b);\n"+
            "}\n" +
    ...
    public CameraCleamProgram(Context context) {
        mDefaultTextureTarget = GLES11Ext.GL_TEXTURE_EXTERNAL_OES;
        mFilterType = FilterType.FILTER_CLEAM;
        mProgramHandle = GlUtil.createProgram(VERTEX_SHADER, FragmentShader);

        if(mProgramHandle == 0) {
            throw new RuntimeException("Unable to create program");
        }
        mInputImageTexture2Handler = createTextureObject(context, R.mipmap.cleam);
        initParams();
    }
