 ...
 */
public class CameraBrightnessProgram extends Texture2dProgram {
    ...
    public CameraBrightnessProgram(Context context, int width, int height){
        super();
        mWidth = width;
        mHeight = height;
        mFilterType = FilterType.BRIGHT_FILTER;
        //TODO
        mProgramHandle = GlUtil.createProgram(VERTEX_SHADER,
                RawResourceReader.readTextFileFromRawResource(context, R.raw.adjust_luminance));
    ...
    protected void initParams(){
        ...
        muRatioLoc = GLES20.glGetUniformLocation(mProgramHandle, "u_luminanceRatio");
    }

    @Override
    public void setRatio(RatioType type, float ratio){
        if(type == RatioType.Default){
            mRatio = 2.0f * (float)(ratio - 0.5);
        }
    }
