 ...
 */
public class CameraSaturationProgram extends Texture2dProgram {
    ...
    public CameraSaturationProgram(Context context, int width, int height){
        super();
        mWidth = width;
        mHeight = height;
        mFilterType = FilterType.SATURATION_FILTER;
        mProgramHandle = GlUtil.createProgram(VERTEX_SHADER,
                RawResourceReader.readTextFileFromRawResource(context, R.raw.adjust_saturation));
    ...
    protected void initParams(){
        ...
        muRatioLoc = GLES20.glGetUniformLocation(mProgramHandle, "u_SaturationRatio");
    }

    @Override
    public void setRatio(RatioType type, float ratio){
        if(type == RatioType.Default){
            mRatio = 2.0f * (float)(ratio - 0.5f);
        }
    }
    ...
    public void draw(float[] mvpMatrix, FloatBuffer vertexBuffer, int firstVertex,
        ...
        GLES20.glUniform1f(muRatioLoc, (float) mRatio);
